...
class TwitterClient():
    ...
    def get_search_tweets(self, num_tweets):
        search_tweets = []
        for tweet in Cursor(self.twitter_client.search, lang="en").items(num_tweets):
            search_tweets.append(tweet)
        return search_tweets

# # # # TWIITER AUTHENTICATOR # # # #
...
class TwitterListener(StreamListener):
    ...
    def on_error(self, status):
        if status == 420:
            # returning False on data menthod in case rate limit occurs
            return False 
        print(status)
