...
class TweetsCorpusReader(CorpusReader):
    ...
    def __init__(self, root, fileids=None, word_tokenizer=TweetTokenizer(), encoding='utf-8-sig'):
    ...
    def docs(self, fileids=None, bullying_trace=None):
        ...
        if bullying_trace:
            new_tweets = [tweet for tweet in tweets if str(tweet['bullying_trace']) =='yes']
        else:
            remove = ['nan','remove']
...
#         """
#         Returns a generator of tweets where each tweet is a list of
...
#         for token in self.tweets(fileids):
#             yield token
...
#         """
#         tagged_tweets =  self.tagged(fileids)
#         return [word[0] for tweet in tagged_tweets for word in tweet]
